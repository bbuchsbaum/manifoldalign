---
title: Fused-Partial Gromov-Wasserstein for Domain Alignment
output: rmarkdown::html_vignette
vignette: '%\VignetteIndexEntry{Fused-Partial Gromov-Wasserstein for Domain Alignment}
...
The total variation penalty provides a soft constraint on transported mass:
...
The `omega1` parameter controls the balance between feature and structural alignment:
...
Apply preprocessing to normalize features before alignment:

```{r preprocessing}
# Create preprocessing pipeline
preproc <- multivarious::prep(
  multivarious::center(),  # Center features
  multivarious::scale()    # Scale to unit variance
)

# This would apply preprocessing if supported
# result_preproc <- fpgw(hd, omega1 = 0.5, preproc = preproc)
```

## References

- Bai et al. (2025). Fused-Partial Gromov-Wasserstein Distance for Heterogeneous Domain Adaptation. arXiv preprint.
