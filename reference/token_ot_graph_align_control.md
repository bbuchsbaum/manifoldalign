...
- n_levels:
  ...
  Integer number of hierarchy levels. \`1\` disables multilevel
  refinement.

- coarsen_ratio:

  Numeric factor \> 1 controlling the coarse-to-fine size reduction
  (roughly \`n_next ≈ n_prev / coarsen_ratio\`).

- coarsen_method:

  Coarsening strategy for multilevel: \`"kmeans"\` (fast, feature-based)
  or \`"louvain"\` (graph community-based with fallback to k-means if
  the number of communities is unsuitable).
...
- prior_mode:
  ...
  Prior lifting mode for multilevel: \`"none"\`, \`"hard"\` (use
  discrete coarse assignment), or \`"soft"\` (use top-k coarse coupling
  mass).

- prior_cluster_k:

  Integer number of coarse target clusters kept per coarse source
  cluster when \`prior_mode="soft"\`.

- prior_max_candidates:

  Maximum prior candidates lifted per fine node (cap to prevent
  blowups).
...
- graph_sigma:
  ...
  Optional numeric bandwidth for heat-kernel affinities. Use \`NULL\` to
...
- view_weights:
  ...
  Optional numeric vector of nonnegative weights, same length as
  \`views\` (will be normalized to sum to 1).

- k_embed:

  Optional integer number of eigenpairs used for eigenmap/HKS. If
  \`NULL\`, defaults to \`max(3\*ncomp, ncomp + 10)\` inside the solver.
...
- ann_backend:
  ...
  Nearest-neighbor backend for candidate generation: \`"auto"\`
  (default), \`"rann"\`, or \`"annoy"\` (requires RcppAnnoy).
...
- ensure_cols:
  ...
  Logical; ensure every target column has at least one incoming
  candidate edge (adds reverse-1NN edges if needed).

- token_mode:

  Either \`"view_only"\` (self token only) or \`"view_plus_neighbors"\`
  (includes sampled neighbor tokens).
...
- anchor_penalty:
  ...
  Nonnegative penalty added to costs for anchor mismatch edges when both
